...
- 书签增删改后自动更新，无需重启浏览器
- 在任意网页按快捷键（默认 **Cmd+Shift+G**）打开标签页搜索窗口，可搜索并快速切换已打开的标签页
...
| 操作 | 说明 |
|------|------|
| 默认快捷键 | Mac：`Cmd+Shift+G` |
| 打开面板 | 在普通网页按下快捷键，搜索框出现在鼠标光标附近 |
| 搜索 | 按标题、URL 或书签名过滤 |
| 切换标签 | ↑↓ 选择，Enter 跳转，Esc 关闭 |
| 自定义快捷键 | 设置页「标签页搜索」→「录制快捷键」 |
| 快捷键无效时 | 设置页点击「测试打开」验证功能；或在 `chrome://extensions/shortcuts` 手动分配 |
...
- 无法作用于 `chrome://` 等系统页面（标签页搜索弹窗同样无法在系统页唤起）
- 需要 `<all_urls>` 权限以在所有书签站点上注入脚本
- Windows/Linux 默认快捷键为 `Ctrl+Shift+G`，可在设置页自定义
- `Cmd+G` 与 Chrome 内置「查找下一个」冲突，不建议使用；请用默认 `Cmd+Shift+G`
...
10. 在设置页点击「测试打开」，确认标签页搜索窗口出现并可搜索、切换标签
11. 按 `Cmd+Shift+G` 确认快捷键可唤起搜索窗口
